...
export class TrainingController {
  ...
  public async savingRelation() {
    ...
    const subject = await this.subjectRepository.find({ where: { id: 3 } });
    ...
    const teacher1 = await this.teacherRepository.find({ where: { id: 5 } });
    const teacher2 = await this.teacherRepository.find({ where: { id: 6 } });
